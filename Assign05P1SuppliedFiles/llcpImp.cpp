...
void PromoteTarget(Node*& headPtr, int target)
{
   ...
   while (cursor)
   {
      // Check if target is found and keep count
   ...
   if (headPtr == 0)
   {
      // Case 1: If list is empty, add node with target
      Node *newNodePtr = new Node;
   ...
   {
      // Case 2: If target isn't found, append new node to end of list
      Node *newNodePtr = new Node;
      newNodePtr->data = target;
      newNodePtr->link = 0;
   ...
   {
      // Case 3: Prepare to promote target!
      cursor = headPtr;
      prev = 0;
      if (headPtr->data != target)
      {
         // Case 3a: First node is not target
         while (cursor)
         {
            ...
            // Traverse past any target values at
